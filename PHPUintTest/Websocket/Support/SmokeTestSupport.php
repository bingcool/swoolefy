...
final class SmokeTestSupport
{
    ...
    /**
     * TCP 预检：服务未启动时抛出 WebsocketServerUnavailableException。
     */
    public static function ensureServerAvailable(string $host, int $port): void
    {
        ...
        throw new WebsocketServerUnavailableException(
            "WebsocketService not reachable at {$host}:{$port} ({$errstr}). {$hint}"
        );
    }
