...
final class CreateCmdExitAndIoTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->appPath = $this->tmpRoot . '/' . self::TEST_APP_NAME;
    ...
    public function testExistingDirectoryReturnsFailureCode(): void
    {
        ...
        $this->assertDirectoryExists($this->appPath);
    }
    ...
    public function testMissingProtocolReturnsFailureCode(): void
    {
        ...
        $this->assertSame(Command::FAILURE, $code);
        $this->assertDirectoryDoesNotExist($this->appPath);
    ...
    public function testTargetConflictDoesNotOverwriteExistingUserDir(): void
    {
        ...
        $targetPath = $this->appPath;
        $cmd = new class ($marker, $targetPath) extends CreateCmdExitAndIoTestableCreateCmd {
            public function __construct(private string $marker, private string $targetPath)
            {
                parent::__construct();
            }
            ...
            protected function buildApplicationSkeleton(string $appPathDir, string $appName, string $protocol): void
            {
                ...
                file_put_contents($this->targetPath . '/keep.txt', $this->marker);
                parent::buildApplicationSkeleton($appPathDir, $appName, $protocol);
            }
        };
        $cmd->appNameOverride = self::TEST_APP_NAME;
        $cmd->appPathOverride = $targetPath;
        $cmd->startDirOverride = $this->startDir;

        $code = $cmd->runExecute();
        $this->assertSame(Command::FAILURE, $code);
        $this->assertDirectoryExists($targetPath);
        $this->assertSame($marker, file_get_contents($targetPath . '/keep.txt'));
        $this->assertNoStagingLeft();
    }
    ...
    {
        $left = glob(dirname($this->appPath) . '/.swoolefy_create_*') ?: [];
...
class CreateCmdExitAndIoTestableCreateCmd extends CreateCmd
{
    ...
    public function runExecute(): int
    {
        ...
        $input = new ArrayInput([
            'app_name' => $this->appNameOverride ?? CreateCmdExitAndIoTest::TEST_APP_NAME,
        ], $definition);
