 ...
 */
final class WorkflowPhase2Test extends TestCase
{
    /**
     ...
     */
    public function testRuleRouter(): void
    {
        ...
        $this->assertTrue($router->route($ctx) === ['coding'], 'RuleRouter should select coding for tech topic');
    }
     ...
     */
    public function testStreamBridgeCollectsEvents(): void
    {
        $run = function (): void {
            ...
            $this->assertTrue(count($sink->events()) === 2, 'StreamBridge should forward two events');
            StreamBridge::unbind();
        };
     ...
     * 确保指标插件正确挂钩引擎生命周期。
     */
    public function testMetricsPluginCountsNodes(): void
    {
        ...
        $this->assertTrue(($snapshot['runs'] ?? 0) >= 1, 'MetricsPlugin should count runs');
        $this->assertTrue(($snapshot['nodes'] ?? 0) >= 1, 'MetricsPlugin should count nodes');
    }
     ...
     */
    public function testWorkflowRegistry(): void
    {
        $registry = new WorkflowRegistry();
        $registry->register('order_processing', static fn () => OrderProcessingFixtureWorkflow::definition(new NeuronFactory()));
        $this->assertTrue($registry->has('order_processing'), 'Registry should know order_processing');
        $compiled = $registry->compiled('order_processing');
        $this->assertTrue($compiled->workflowId() === 'order_processing', 'Compiled workflow id should match');
    }
     ...
     */
    public function testMultiAgentResearchFixtureWorkflow(): void
    {
        ...
        $this->assertTrue($run->status->value === 'completed', 'Research workflow should complete');
        $this->assertTrue(count($run->state->agentOutputs) === 2, 'Should have coding and finance outputs');
        $this->assertTrue(isset($run->state->data['summary']), 'Summary node should write data.summary');
    }
}
